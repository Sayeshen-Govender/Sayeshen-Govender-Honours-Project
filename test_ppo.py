...
#Customized version of CleanRL evaluate function for PPO.
def evaluate(model_path: str,make_env: Callable,env_id: str,eval_episodes: int,run_name: str,Model: torch.nn.Module,
             device: torch.device = torch.device("cpu"),capture_video: bool = True,gamma: float = 0.99,
             gravity_scale: float = 1.0, mass_scale: float = 1.0):
    ...
    #Debug block
    raw_env = envs.envs[0].unwrapped  #Unwrap the first env to access MuJoCo model
    ...
    # Initialize the agent and load trained model weights
